...
df_price.rename(columns={'etime': 'timestamp'}, inplace=True)  # 对列名进行替换
# print(df_price)
...
del df_price['index']  # 这一步也可以用df.reset_index(drop=True)，去除原索引
df_price = df_price.set_index('timestamp')  # 用timestamp这一列重置索引
df_price['year'] = df_price.index.year  # 新增一列，这一列是年份
