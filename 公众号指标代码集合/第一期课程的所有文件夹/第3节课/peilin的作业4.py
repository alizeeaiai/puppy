...
def MA_Strategy(data_price, window_short=5, window_median=10, window_long=20, loss_ratio=0.20):
    ...
    data_price['cdp'] = data_price['high'] + \
        data_price['low'] + data_price['close']
    data_price['cdp_al'] = 2*data_price['cdp'] - \
        2*data_price['high']+data_price['low']
    ...
    for i in range(max(1, window_long), data_price.shape[0]):
        ...
        # 作业四 , 加入此条件后，nav从400多变为不到1，所以此条件很不好。
        elif (data_price['position'][i-1] == 1) and (data_price['close'][i] < data_price['cdp_al'][i]):
            # print(i - entry_index, ' hodling for a period')
            data_price['flag'][i] = -1
            data_price['position'][i] = 0  # 仓位记录为0，表示没有仓位了
            date_out = data_price.index[i]  # 记录卖出的时间 年-月-日
...
trans, data = MA_Strategy(df_price, window_short=3,
                          window_median=8, window_long=13, loss_ratio=0.05)
