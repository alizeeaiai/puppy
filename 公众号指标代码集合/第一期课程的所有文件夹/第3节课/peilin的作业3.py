...
def MA_Strategy(data_price, window_short=5, window_median=10, window_long=20, loss_ratio=0.20):
    ...
    data_price['atr'] = ta.ATR(
        data_price['high'], data_price['low'], data_price['close'])
    ...
    for i in range(max(1, window_long), data_price.shape[0]):
        ...
        # 作业三，发现atr方法不如原方法，N倍ATR的n调整不同参数都只能让最终回报达到200多倍，但原方法（上行被注释的方法）是达到466倍。
        elif (data_price['position'][i-1] == 1) and ((max(data_price.high[i-5:i]) / data_price['close'][i] - 1) > data_price['atr'][i]*2):
