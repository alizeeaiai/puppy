...
if api.connect('119.147.212.81', 7709):  # 注意这里的IP地址和数据接口
    current_data = api.to_df(
        api.get_security_bars(4, 1, '510050', 1, 200))  # 注意这里，第一个1表示是15分钟的数据，其中0为5分钟K线 1 15分钟K线 2 30分钟K线 3 1小时K线 4 日K线
...
df_price = df_price.drop_duplicates('timestamp').reset_index()  # 注意这一步非常重要，以timestamp为基准进行去重处理
del df_price['index']
...
def MA_Strategy(data_price, window_short=5, window_median=10, window_long=20, loss_ratio=0.20):
    ...
    data_price['kdj_k'], data_price['kdj_d'] = ta.STOCH(data_price['high'], data_price['low'], data_price['close'],
    ...
    data_price['slope'] = ta.LINEARREG_SLOPE(data_price['lma'], timeperiod=5)  # 作业3：5个周期的20日均线斜率
    data_price['atr'] = ta.ATR(data_price['high'], data_price['low'], data_price['close'], timeperiod=14)
    data_price['CDP'] = (data_price['low'] + data_price['high'] + data_price['close']) / 3
    data_price['AL'] = data_price['CDP'] * 2 - data_price['high'] * 2 + data_price['low']
    # 计算CDP的AL
    ...
    y = list(
        data_price.loc[:, ['open', 'high', 'low', 'close']].round(4).values)  # 设置为list，一共有data_price.shape[0]个，等待传入Kbar
    ...
    for i in range(max(1, window_long), data_price.shape[0]):
        ...
        buy_cond_1 = (data_price['sma'][i - 1] < data_price['lma'][i - 1]) and (
                    data_price['sma'][i] > data_price['lma'][i])  # 金叉
        buy_cond_2 = (data_price['lma'][i] > data_price['lma'][i - 1]) and (
                    data_price['sma'][i - 1] > data_price['mma'][i - 1]) and (
                             data_price['sma'][i] < data_price['mma'][i]) and (
                                 data_price['sma'][i] > data_price['lma'][i])
        buy_cond_3 = (data_price['slope'] > 0)  # 5个周期20日均线向上
        ...
        # 第二个作业：开发N倍ATR作为高点回落出局条件
                ...
                (max(data_price.high[i - 5:i]) / data_price['close'][i] - 1) > loss_ratio * 0.8):
        ...
        # 第二个作业：开发N倍ATR作为高点回落出局条件
                ...
                (max(data_price.high[i - 5:i]) - data_price['close'][i]) > data_price['atr'][
            i] * 2):  # 收盘价比前高下跌2倍atr则止损或者止盈
            print(data_price.index[i], '============', max(data_price.high[i - 5:i]), '=========2倍atr回落平仓')
            data_price['flag'][i] = -1  # 记录做多还是做空，这里-1是卖出
            data_price['position'][i] = 0  # 仓位记录为0，表示没有仓位了
            date_out = data_price.index[i]  # 记录卖出的时间 年-月-日
            price_out = data_price['close'][i]  # 记录卖出的价格，这里是以收盘价卖出
            Sell.append([date_out, price_out, '0.8倍atr止盈'])  # 把卖出记录保存到Sell列表里

        # 情形六：买入后三个bar还不赚钱立马平仓  计步器
        elif (data_price['position'][i - 1] == 1) and (i - entry_index > 3) and (
                data_price['close'][i] / price_in < 0.985):
            # print(i - entry_index, ' hodling for a period')
            data_price['flag'][i] = -1  # 记录做多还是做空，这里-1是做空
            data_price['position'][i] = 0  # 仓位记录为0，表示没有仓位了
            date_out = data_price.index[i]  # 记录卖出的时间 年-月-日
            price_out = data_price['close'][i]  # 记录卖出的价格，这里是以收盘价卖出
        ...
        # 情形七：盘中击穿重要支撑立马平仓
        # 作业4：把CDP指标写入到策略中，加入一个止损条件：当日价格如果击穿CDP的AL，也就是下方支撑，立即止损
        elif (data_price['position'][i - 1] == 1) and (data_price['low'][i] < data_price['AL'][i - 1]):
            data_price['flag'][i] = -1  # 记录做多还是做空，这里-1时做空
            data_price['position'][i] = 0  # 仓位记录为1，表示有1手仓位
            date_out = data_price.index[i]  # 记录买入的时间 年-月-日
            price_out = data_price['AL'][i - 1] * 0.998  # 记录卖出的价格，这里是以跌破AL下方0.2%止损，一旦跌破AL，就假设在其下方0.2止损
            entry_index = i
            print(data_price.index[i], '=========跌破AL止损@--', price_out)
            Sell.append([date_out, price_out, '跌破AL止损'])  # 把卖出记录保存到Sell列表里

        # 情形八：逆势买入获得优势成本
        elif (data_price['position'][i - 1] == 0) and (data_price['kdj_k_ma5'][i] > 80) and (
                data_price['kdj_k'][i - 1] > 80) and (data_price['kdj_k'][i] < 80):
            data_price['flag'][i] = 1  # 记录做多还是做空，这里1是做多
            data_price['position'][i] = 1  # 仓位记录为1，表示有1手仓位
            date_in = data_price.index[i]  # 记录买入的时间 年-月-日
            price_in = data_price['close'][i]  # 记录买入的价格，这里是以收盘价买入
