...
file_path = '/Users/aiailyu/PycharmProjects/第3节课/'
...
if api.connect('119.147.212.81', 7709): # 注意这里的IP地址和数据接口
    current_data = api.to_df(api.get_security_bars(4, 1, '510050', 1, 200)) # 注意这里，第一个1表示是15分钟的数据，其中0为5分钟K线 1 15分钟K线 2 30分钟K线 3 1小时K线 4 日K线
...
#slope 20日均线
df_price['slope'] = ta.LINEARREG_SLOPE(df_price['close'], timeperiod=20)
df_price = df_price.replace([np.nan, np.inf, -np.inf], 0.0)

highs = np.array(df_price['high'])
lows = np.array(df_price['low'])
closes = np.array(df_price['close'])
# 问题一：Ta.ATR() 其中timeperiod表示的是周期 一般是14或者21为周期。N倍是自己设定的，和周期不一样
df_price['atr'] = ta.ATR(highs, lows, closes, timeperiod=14)
df_price = df_price.replace([np.nan, np.inf, -np.inf], 0.0)
df_price['CDP'] = (df_price['high'].shift(1)+df_price['low'].shift(1)+
                         df_price['close'].shift(1))/3
df_price['AL'] = 2*df_price['CDP'] -2*df_price['high'].shift(1) + df_price['low'].shift(1)
df_price['high_shift'] = df_price['high'].shift(1)
print(df_price)
...
def MA_Strategy(data_price, window_short=5, window_median=10, window_long=20, loss_ratio=0.20):
    ...
    for i in range( max(1,window_long),data_price.shape[0]):
        ...
        buy_cond_1 = (data_price['sma'][i - 1] < data_price['lma'][i - 1]) and (data_price['sma'][i] > data_price['lma'][i])
        ...
        elif (data_price['position'][i-1] == 1) and (i - entry_index > 2) and (data_price['close'][i] / price_in < 0.985 ) :
        ...
        #N倍atr止损
        # 问题三：止损条件逻辑
        # elif (data_price['position'][i-1] == 1) and (price_in - data_price['close'][i] <= -data_price['atr'][i] * 0.8)
        elif (data_price['position'][i-1] == 1) and (price_in - data_price['close'][i] >= data_price['atr'][i] * 0.8) :
            data_price['flag'][i] = -1  # 记录做多还是做空，这里-1是做多
            data_price['position'][i] = 0  # 仓位记录为0，表示没有仓位了
            date_out = data_price.index[i]  # 记录卖出的时间 年-月-日
            price_out = data_price['close'][i]  # 记录卖出的价格，这里是以收盘价卖出
            Sell.append([date_out, price_out, '0.8倍atr止损'])  # 把卖出记录保存到Sell列表里

        # 情形七：盘中击穿重要支撑立马平仓
        elif (data_price['position'][i-1] == 1) & (data_price['close'][i] < data_price['AL'][i]) :
            data_price['flag'][i] = -1  # 记录做多还是做空，这里-1是做多
            data_price['position'][i] = 0  # 仓位记录为0，表示没有仓位了
            date_out = data_price.index[i]  # 记录卖出的时间 年-月-日
            price_out = data_price['close'][i]  # 记录卖出的价格，这里是以收盘价卖出
            Sell.append([date_out, price_out, '盘中击穿重要支撑平仓'])  # 把卖出记录保存到Sell列表里
        ...
        # elif (data_price['position'][i-1] == 0) and (data_price['kdj_k'][i] > 80):
        #     data_price['flag'][i] = 1  # 记录做多还是做空，这里1是做多
        #     data_price['position'][i] = 1  # 仓位记录为1，表示有1手仓位
        #     date_in = data_price.index[i]  # 记录买入的时间 年-月-日
        #     price_in = data_price['close'][i]  # 记录买入的价格，这里是以收盘价买入
        #     entry_index = i
        #     Buy.append([date_in, price_in, 'kdj大于80买入'])
...
def show_performance( transactions,strategy ):
    ...
    #年化收益率
    N = 250
    rety = strategy.nav[strategy.shape[0] - 2]**(N/strategy.shape[0]) - 1
    print(strategy.nav[strategy.shape[0] - 2], '这是明明')
    print(N/strategy.shape[0], '这是红红')
