...
def backtest(original_data, index_code, frequency, n_days):
    ...
    # 计算实际时间步的索引间隔
    if frequency == '15': # 修改为15
        t_delta = int(1 * n_days) # n_days在这里首次出现
    else:
        t_delta = int(int(240 / int(frequency)) * n_days)

    final_frame['close_nperiod'] = final_frame['close'].shift(-1)
    final_frame['ret'] = (final_frame['close_nperiod']/final_frame['close']) - 1

    # 遍历计算未来n天的收益率（注意行索引的上限）
    # for i in range(0, len(final_frame) - t_delta, 1):
    #     final_frame.at[i, 'ret'] = final_frame.at[i + t_delta, 'close'] / final_frame.at[i, 'close'] - 1
    ...
    # 查询2019-12-31 15:00:00对应的索引（训练集和测试集的划分界限）
    train_set_end_index = data_for_model[(data_for_model['etime'].dt.year == 2016) & (data_for_model['etime'].dt.month == 12) & (data_for_model['etime'].dt.day == 30) & (data_for_model['etime'].dt.hour == 15)].index.values[0] #  & (data_for_model['etime'].dt.hour == 15)
    ...
    # 划分训练集、测试集
    X_train = data_for_model.loc[: train_set_end_index, 'fct'].values.reshape(-1, 1) # reshape成为一列
    ...
    #print(end_index) # start_index和end_index也都是二维数组，所以使用values[0]获得其中的数字
    ...
    ret_frame_train_total['position'] = [(i / 0.0005) * 0.01 for i in y_train_hat]  # 训练值每间隔0.0005对应仓位变化1%
    ...
    for i in range(0, len(ret_frame_train), 1):
        ...
        # 计算持仓净值
        if i == 0 or ret_frame_train.at[i - 1, 'position'] == 0:  # 如果是第一个时间步或前一个区间的结束时刻为空仓状态
            ret_frame_train.at[i, '持仓净值'] = 1
        else:
            close_2 = ret_frame_train.at[i, 'close']
            close_1 = ret_frame_train.at[i - 1, 'close']
            position = abs(ret_frame_train.at[i - 1, 'position'])  # 获取仓位大小（上一周期）
            

            if ret_frame_train.at[i - 1, 'position'] > 0:  # 如果上一周期开的是多仓 之前是i-1，暂时删除试试
                ret_frame_train.at[i, '持仓净值'] = 1.0 * (close_2 / close_1)
            elif ret_frame_train.at[i - 1, 'position'] < 0:  # 如果上一周期开的是空仓
                ret_frame_train.at[i, '持仓净值'] = 1.0 * (1 - (close_2 / close_1 - 1))
                # 这些程序全都是调整仓位，而不是long or short，比较有意思
                # 可以在这个基础上：if ret_frame_train.at[i - 1, 'position'] > 0 进行修改，如果预测的是开多，仓位映射，
                #*********注意，这里应该是i，而不是i-1，否则会出很大的错
                

    # 3：滚动测算累计持仓净值
    ret_frame_train.at[0, '持仓净值（累计）'] = 1
    ...
    ret_frame_test_total['position'] = [(i / 0.0005) * 0.01 for i in y_test_hat]  # 预测值每间隔0.0005对应仓位变化1%
    ...
    for i in range(0, len(ret_frame_test), 1):
        ...
        if i == 0 or ret_frame_test.at[i - 1, 'position'] == 0:  # 如果是第一个时间步或前一个区间的结束时刻为空仓状态
            ...
            if ret_frame_test.at[i - 1, 'position'] > 0:  # 如果上一周期开的是多仓
                #ret_frame_test.at[i, '持仓净值'] = 1 * (close_2 / close_1) * position + 1 * (1 - position)
                ret_frame_test.at[i, '持仓净值'] = 1.0 * (close_2 / close_1)
            elif ret_frame_test.at[i - 1, 'position'] < 0:  # 如果上一周期开的是空仓
                #ret_frame_test.at[i, '持仓净值'] = 1 * (1 - (close_2 / close_1 - 1)) * position + 1 * (1 - position)
    ...
    ret_frame_train_test_total['position'] = [(i / 0.0005) * 0.01 for i in y_train_hat] + [(i / 0.0005) * 0.01 for i in y_test_hat]  # 训练值每间隔0.0005对应仓位变化1% + 预测值每间隔0.0005对应仓位变化1%
    ...
    for i in range(1, len(ret_frame_train_test), 1):
        ret_frame_train_test.at[i, '持仓净值（累计）'] = ret_frame_train_test.at[i - 1, '持仓净值（累计）'] * ret_frame_train_test.at[i, '持仓净值']
    ...
    for i in range(1, len(net_asset_value_frame), 1):
        net_asset_value_frame.at[i, 'daily_log_return'] = math.log(net_asset_value_frame.at[i, 'nav']) - math.log(net_asset_value_frame.at[i - 1, 'nav'])  # 计算对数收益率（日度）
    ...
    for i in range(1, len(ret_frame_train_test), 1):
        delta_value = ret_frame_train_test.at[i, '持仓净值（累计）'] - ret_frame_train_test.at[i - 1, '持仓净值（累计）']  # 计算每次交易过程中累计持仓净值的变化量
