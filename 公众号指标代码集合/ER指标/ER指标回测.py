...
def my_strategy(data_price):
    ...
    for i in range(1, data_price.shape[0]):
        # 买入情形:当ER指标连续3个交易日上涨，买入
        if (data_price['position'][i - 1] == 0) & (data_price['er'][i] > 0.7) & (data_price['er'][i-1] > 0.7) & (data_price['er'][i-2] > 0.7) & (data_price['close'][i] > data_price['close'][i-1]) & (data_price['close'][i-1] > data_price['close'][i-2]):
            ...
            entry_index = i
            print(data_price.index[i], price_in)
            Buy.append([date_in, price_in,'er连续涨3个交易日，买入'])  # 把买入记录保存到Buy列表里
            # 上述也都可以使用data_price.at[i, 'position']的用法，为了确保没有错误，暂且这么使用

        # 卖出情形：当ER指标连续3个交易日下跌，卖出
        elif data_price['position'][i - 2] == 1:
            ...
            Sell.append([date_out, price_out, 'er连续跌3个交易日，卖出'])  # 把卖出记录保存到Sell列表里
